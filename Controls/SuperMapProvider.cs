...
namespace GMap.SuperMapProvider
{
    ...
    public class SuperMapProvider : GMapProvider
    {
        ...
        public SuperMapProvider(string mapServiceUrl, string mapName, double[] mapScales)
        {
            this._map = new SuperMap.Connector.Map(mapServiceUrl);
            this._serviceUrl = mapServiceUrl;
            this._mapName = mapName;
            MapParameter defaultMapParameter = this._map.GetDefaultMapParameter(mapName);
            _mapScales = mapScales;
            _superMapProjection = new SuperMapProjection(mapScales, defaultMapParameter);
        }

        private double[] _mapScales = null;
        private SuperMapProjection _superMapProjection = null;
        ...
        private string MakeTileImageUrl(GPoint pos, int zoom, string language)
        {
            ...
            //if (_mapScales.Length >= zoom && zoom > 0)
            //{
            //    scale = _mapScales[zoom - 1];
            //}
        ...
        private PureProjection _projection = MercatorProjection.Instance;
        public override PureProjection Projection
        {
            get { return _superMapProjection; }
        }
    }
}
